import React from "react";
import ReactStars from "react-rating-stars-component";
import { Link } from "react-router-dom";

const SpecialProduct = () => {
  return (
    <div className="col-6 mb-3">
      <div className="special-product-card">
        <div className="d-flex justify-content-between">
          <div>
            <img
              src="images/watch.jpg"
              className="img-fluid"
              alt="productImage"
            ></img>
          </div>
          <div className="special-product-content">
            <h5 className="brand mb-1">Havells</h5>
            <h6 className="title">Samsung Galaxy Note+ camera phone<br/>Snapdragon 8 gen3 mobile phones</h6>
            <ReactStars
              count={5}
              size={24}
              value={4}
              edit={false}
              activeColor="#ffd700"
            />
            <p className="price">
              <span className="red-p">₹100</span> &nbsp; <strike>₹200</strike>
            </p>
            <div className="discount-till d-flex align-items-center gap-10">
              <p className="mb-0">
                <b>5 </b>Days
              </p>
              <div className="d-flex gap-10 align-items-center">
                <span className="badge rounded-circle p-2 bg-danger">10</span>;
                <span className="badge rounded-circle p-2 bg-danger">11</span>;
                <span className="badge rounded-circle p-2 bg-danger">12</span>
              </div>
            </div>
            <div className="prod-count my-2">
              <p>Product: 5</p>

              <div className="progress" style={{ height: "8px",width:"85%" }}>
                <div
                  className="progress-bar"
                  role="progressbar"
                  style={{ width: "25%" }}
                  aria-valuenow="25"
                  aria-valuemin="0"
                  aria-valuemax="100"
                ></div>
              </div>
            </div>
            <Link className="button mt-1">Add to Cart</Link>
          </div>
        </div>
      </div>
    </div>
  );
};

export default SpecialProduct;
